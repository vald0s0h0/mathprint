...
Deux couches, dans cet ordre :
  ...
  1. `lint` — contrôles STRUCTURELS et de CLARTÉ, toujours appliqués. Ils ne
     ...
     QCM, énoncé qui renvoie hors de la feuille (« voir page 42 »), figure
     mentionnée sans figure disponible.
...
from .grading import TRANSFORMS, normalize, parse_number
...
# qui s'appuie sur le manuel, sur l'exercice voisin ou sur « le tableau
# ci-dessus » est insoluble une fois imprimé seul sur une carte.
_EXTERNAL_REF_RE = re.compile(
    r"(?i)\b(voir\s+(la\s+)?page|à\s+la\s+page|page\s+\d+|"
    r"exercice\s+(n[°o]\s*)?\d+|question\s+précédente|"
    r"ci[- ]dessus|précédemment|comme\s+dans\s+l['’]exercice)\b")

# Mention d'un visuel. Reprise TELLE QUELLE de la détection de services.indigo
# (même règle des deux côtés : si l'énoncé parle d'une figure, il en faut une).
...
def lint(variant: dict, *, has_figure: bool) -> list[str]:
    ...
    statement = str(variant.get("statement") or "").strip()
    ...
    ref = _EXTERNAL_REF_RE.search(statement)
    if ref:
        problems.append(f"renvoi hors de la feuille (« {ref.group(0)} ») : l'élève "
                        "n'a que sa copie, l'énoncé doit être autonome")
    if not has_figure and _FIGURE_REF_RE.search(statement):
        problems.append("l'énoncé parle d'une figure alors qu'aucune image n'est "
                        "disponible : reformule sans visuel, ou décris les données")
...
def _lint_choices(variant: dict, *, exclusive: bool) -> list[str]:
    ...
    if not 2 <= n <= scoring.QCM_MAX_CHOICES:
        return [f"{n} proposition(s), attendu 2 à {scoring.QCM_MAX_CHOICES} "
                f"(le barème codé vaut {scoring.QCM_BOX_POINTS} point par case)"]
...
def _lint_grid(variant: dict) -> list[str]:
    ...
    if not isinstance(rows, list) or not 2 <= len(rows) <= scoring.QCM_MAX_GRID_ROWS:
        ...
        return problems + [f"{n} ligne(s), attendu 2 à {scoring.QCM_MAX_GRID_ROWS} "
                           f"(le barème codé vaut {scoring.QCM_BOX_POINTS} point par ligne)"]
    labels = []
...
def _eval(expr: str):
    ...
    try:
        return sympy.simplify(parse_expr(str(expr), transformations=TRANSFORMS))
...
def _truth(expr) -> bool | None:
    ...
    if isinstance(expr, bool):
        return expr
    try:
        value = sympy.sympify(str(expr))     # ASCII sympy, cf. _eval
    except Exception:
        return None
...
def verify(variant: dict, *, has_figure: bool) -> list[str]:
    ...
    problems = lint(variant, has_figure=has_figure)
    if problems:
        return problems
