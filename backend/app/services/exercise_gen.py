...
_FORMAT_MENU = (
    ...
    "valeurs, un tableau de proportionnalité), OU quand un même énoncé "
    "engendre plusieurs sous-questions STRUCTURELLEMENT IDENTIQUES (même "
    "forme de phrase, seuls les nombres changent) — c'est le format À "
    "PRIVILÉGIER pour ces listes régulières plutôt que des sous-questions "
    "a./b./c. en vrac. Mise en page : un vrai tableau à cadre, CENTRÉ, "
    "1re colonne = l'énoncé de la ligne (row_labels[i]), colonne(s) suivante(s) "
    "= la ou les cases à remplir. Règles de rédaction des libellés :\n"
    "  • row_labels[i] porte l'énoncé COURT et AUTONOME de la ligne, avec le "
    "trou là où l'élève écrit (ex. « 7 × 8 = », « Périmètre d'un carré de "
    "côté 5 cm »). NE PRÉFIXE PAS par « a. »/« b. »/« c. » ni par un numéro : "
    "la grille numérote déjà visuellement les lignes.\n"
    "  • cols=1 dans le cas dominant (énoncé | réponse). Passe à cols≥2 "
    "seulement pour une VRAIE grille de valeurs (ex. x en 1re colonne "
    "\"given\", f(x) à remplir en 2e) : donne alors col_labels pour coiffer "
    "chaque colonne.\n"
    "  • cells[i][j] = la réponse attendue de la case (row-major, dans "
    "l'ordre des lignes puis des colonnes).\n"
    "  • garde les libellés BREFS : un tableau fin à beaucoup de lignes est "
    "automatiquement scindé en deux demi-tableaux côte à côte pour gagner de "
    "la place, ce qui suppose des libellés qui tiennent sur une largeur "
    "réduite.\n"
    "\"statement\" ne porte que la consigne commune (souvent très courte, ex. "
    "« Calcule. », « Complète le tableau. »), tout le détail est dans "
    "row_labels/col_labels. Si un tableau du manuel imprime déjà certaines "
    "valeurs (colonne de calcul donnée, seule la colonne résultat est à "
...
_BLANK_WRITING_RULES = (
    ...
    "MARQUEUR — impératif : la case s'écrit EXACTEMENT « {{blank}} », en toutes "
    "lettres, dans le fil du texte. JAMAIS le mot « blank » seul, jamais entre "
    "simples accolades « {blank} », et surtout JAMAIS à l'intérieur d'une "
    "formule $...$ : « {{blank}} » est du TEXTE, il reste HORS des bornes $...$. "
    "Un nombre collé à la case s'écrit donc « $85${{blank}} » (le nombre en "
    "formule, la case en texte juste après) et NON « $85\\text{blank}$ » ni "
    "« $85blank$ » — sinon la case ne s'imprime pas et « blank » s'affiche en "
    "toutes lettres dans l'énoncé.\n"
